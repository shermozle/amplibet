...
const FeaturedEvents: React.FC = () => {
  return <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      {mockSportEvents.map(event => <Link key={event.id} to={`/event/${event.id}`} className="bg-surface rounded-lg overflow-hidden hover:bg-raised transition-colors">
          ...
          <div className="p-4">
            ...
            <div className="mb-4">
              <div className="flex items-center mb-1">
                <div className="w-6 h-6 rounded-full bg-raised flex items-center justify-center mr-2">
                  {event.sportId === 'afl' ? '🏉' : '🏉'}
                </div>
            ...
            <div className="flex justify-between">
              <div className="flex-1 bg-raised rounded p-2 mr-1 text-center">
                <div className="text-xs mb-1">
                  {event.homeTeam.split(' ')[0]}
                </div>
                <div className="font-bold">{event.odds.home.toFixed(2)}</div>
              </div>
              {event.odds.draw && <div className="flex-1 bg-raised rounded p-2 mx-1 text-center">
                  <div className="text-xs mb-1">Draw</div>
                  <div className="font-bold">{event.odds.draw.toFixed(2)}</div>
                </div>}
              <div className="flex-1 bg-raised rounded p-2 ml-1 text-center">
                <div className="text-xs mb-1">
