...
const BetSlip: React.FC = () => {
  ...
  if (isCollapsed) {
    return <div className="w-12 bg-[#1B3B6F] border-l border-[#13294B] flex flex-col items-center py-4">
        <button onClick={() => setIsCollapsed(false)} className="w-10 h-10 bg-[#2A4E8D] rounded-full flex items-center justify-center mb-2">
          <ClipboardIcon size={18} />
        </button>
        <div className="text-xs font-semibold">{selectedBets.length}</div>
      </div>;
  }
  return <div className="w-72 bg-[#1B3B6F] border-l border-[#13294B] flex flex-col">
      <div className="p-3 border-b border-[#13294B] flex justify-between items-center">
        ...
        {selectedBets.length === 0 ? <div className="p-4 text-center text-gray-400 text-sm">
            ...
            {selectedBets.map(bet => <div key={bet.id} className="p-3">
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <div className="bg-[#2A4E8D] rounded py-1 px-3 text-sm">
                    {bet.odds.toFixed(2)}
                  </div>
                </div>
      ...
      <div className="border-t border-[#13294B] p-3">
        ...
        <button className="w-full bg-[#4F44E0] hover:bg-[#3832A0] text-white py-2 rounded mt-3 font-medium" disabled={selectedBets.length === 0 || totalStake === 0} onClick={placeBets}>
          Place Bet
        </button>
      </div>
    </div>;
